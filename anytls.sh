...
latest_asset(){ local json url re; re="$(asset_regex)"; json="$(curl -fsSL https://api.github.com/repos/anytls/anytls-go/releases/latest)"; url="$(printf '%s' "$json" | grep 'browser_download_url' | cut -d '"' -f 4 | grep -E "$re" | head -n1 || true)"; if [[ -z "$url" && "$(uname -m)" =~ armv7 ]]; then warn "anytls 官方最新 Release 暂无 armv7 预编译包"; err "当前架构 armv7l 暂无可下载资产，请等官方发布或手动编译"; exit 1; fi; [[ -n "$url" ]] || { err "未找到当前架构的 anytls-go Release 资产"; exit 1; }; echo "$url"; }
